...
unsigned long long BitOperations::reduction(unsigned long long digit) {
	return (digit & MersennDigit) + (digit >> 31);
}

unsigned long long BitOperations::mod(unsigned long long digit) {
	return reduction(reduction(digit) + 1) - 1;
}

unsigned long long BitOperations::add(unsigned long long digit1, unsigned long long digit2) {
	return (digit1 += digit2) >= MersennDigit ? (digit1 -= MersennDigit) : digit1;
}

unsigned long long BitOperations::sub(unsigned long long digit1, unsigned long long digit2) {
	return (digit1 -= digit2) >= MersennDigit ? (digit1 += MersennDigit) : digit1;
}

//Counting 1st hash
unsigned long long Hasher::calculateFirstHash(std::string firtsKgram) {
...
//Counting following ring hash
unsigned long long Hasher::calculateNextHash(std::string prevKgram, std::string newKgram) {
	unsigned long long newHash = 0;
	newHash = bitOperator.mod(bitOperator.mod((hashes.back() - prevKgram[0] * x_in_k_mod)*x) + newKgram.back());
	return newHash;
}

//Filling document's hash
void Hasher::fillHashTable(std::string document) {
...
std::vector<std::pair<unsigned long long, int>> Hasher::CreateFingerprint(std::string document) {
...
Hasher::Hasher(std::string document, std::string name1) {
	x_in_k = std::pow(x, k);
	name = name1;
...
PlagResults::PlagResults(double comp1_s, double comp2_s, std::string name1, std::string name2) {
	comp1 = comp1_s * 100;
	comp2 = comp2_s * 100;
...
PlagResults::PlagResults(std::string name1, std::string name2) {
	plag = "Plagiarism didn't found\n";
}

std::string PlagResults::givePlagResults() {
	return plag;
}

// Sorting fingerprints
auto Compare::sortFingerPrint(std::vector<std::pair<unsigned long long, int>>& vect) -> void {
...
//Doing merging lists of fingerprints for finding union of two fingerprints
auto Compare::mergeJoin() -> void {
	int i = 0;
	int j = 0;
...
//Counting preliminary measure of plagiarism
auto Compare::maxContaitment() -> void {
...
// If preliminary measure of plagiarism more than 20%, counting more
//exact measure (going along the smaller document and trying to
//expand boundaries of matching hashes)
auto Compare::WideHash() -> void {
	int i = 0;
	...
	if (maxContaitment2 < 0.25 &&  maxContaitment3 < 0.25)
	{
		PlagResults res2(A.name, B.name);
		res = res2;
		return;
...
Compare::Compare(Hasher a, Hasher b) {
		A = a;
		B = b;
	...
	auto Compare::makeResults() -> PlagResults {
		return res;
	}
